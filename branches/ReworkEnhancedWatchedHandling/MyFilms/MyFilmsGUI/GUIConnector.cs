...
namespace MyFilmsPlugin.MyFilms.MyFilmsGUI
{
    internal class GUIConnector
    {
        ...
        void StopBackgroundTask(bool byUserRequest)
        {
            ...
            {
                LogMyFilms.Debug("Aborting background thread: {0}", _CurrentTaskDescription);
        ...
        internal bool ExecuteInBackgroundAndCallback(Func<object> task, Action<bool, object> resultHandler, string taskDescription, bool timeout)
        {
            ...
            if (!IsBusy && Monitor.TryEnter(this))
            {
                try
                {
                    ...
                    backgroundThread = new Thread(delegate()
                    {
                        ...
                        {
                            if (!abortedByUser) LogMyFilms.Debug("Timeout waiting for results.");
                            Thread.ResetAbort();
                        }
                        catch (Exception threadException)
                        {
                            _CurrentError = threadException;
                            LogMyFilms.Debug(threadException.ToString());
            ...
            {
                LogMyFilms.Debug("Another thread tried to execute a task in background.");
                return false;
            }
        }
        ...
        void ExecuteTaskResultHandler()
        {
            ...
            if (_CurrentTaskSuccess != true)
            {
                ...
                {
                    string[] lines = new string[] { string.Format("{0} {1}", "Error", _CurrentTaskDescription), _CurrentError.Message };
                    MyFilms.ShowDialogOk("MyFilms", lines);                    
                }
                else
                {
                    if (!abortedByUser)
