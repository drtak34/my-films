  ...
  public class MyFilms : GUIWindow, ISetupForm
  {
    ...
    private void GetImages(string wStrSort, string strThumbDirectory, bool isperson, bool getThumbs, bool createFanartDir, bool countItems)
    {
      ...
      new Thread(delegate()
      {
        ...
        if (rtemp != null && conf.StrPersons.Length > 0)
        {
          ...
          try
          {
            #region the fast version without continuous facade updates
            //for (i = 0; i < facadeFilms.Count; i++)
            //{
            //  facadeFilms[i].Label2 = label2NamePrefix + " (" + rtemp.Count(x => x[wStrSort].ToString().Contains((isReversed) ? ReReverseName(facadeFilms[i].Label) : facadeFilms[i].Label)) + ")";
            //}
            #endregion

            #region the slow version with continuous facade updates
            for (int j = 0; j < facadeFilms.Count; j++)
            {
              facadeCounts[j] = 0;
          ...
          catch (Exception ex) { LogMyFilms.Warn("GetCounts() - error setting facadelist item '" + i + "': " + ex.Message); }
          stopwatch.Stop();
    ...
    private void Context_Menu_Movie(int selecteditem)
    {
      ...
      if (facadeFilms.SelectedListItemIndex > -1 && (conf.ViewContext == ViewContext.TmdbMovies || conf.ViewContext == ViewContext.Movie || conf.ViewContext == ViewContext.MovieCollection))
      {
        ...
        if (MyFilmsDetail.ExtendedStartmode("Context: TMDB Context Search for FyFilms (local)"))
        {
          if (conf.ViewContext == ViewContext.TmdbMovies)
          {
            dlg.Add(GUILocalizeStrings.Get(10798601)); // Search MyFilms ... 
            updChoice.Add("searchmyfilms");
          }
        }
      }
      #endregion
      ...
      if (facadeFilms.SelectedListItemIndex > -1 && (conf.ViewContext != ViewContext.Menu && conf.ViewContext != ViewContext.MenuAll && conf.ViewContext != ViewContext.TmdbMovies))
      {
      ...
      switch (updChoice[dlg.SelectedLabel])
      {
        ...
        case "moviedetailsweb": // search movie URL via grabber scripts and show webpage
          {
    ...
    private void SearchMoviesbyProperties(bool returnToContextmenu, string searchExpression, string searchField) // Old hardcoded searchlist: "TranslatedTitle|OriginalTitle|Description|Comments|Actors|Director|Producer|Rating|Year|Date|Category|Country"
    {
      ...
      {
        #region interactive search menu, when no expression available
        if (dlg == null) return;
        dlg.Reset();
        ...
        #region let user select the seaqrch option
