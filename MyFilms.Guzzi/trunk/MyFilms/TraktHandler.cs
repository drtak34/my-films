  ...
  public static class TraktHandler
    {
        ...
        public static TraktMovieScrobble CreateScrobbleData(IMDBMovie movie)
        {
            ...
            TraktMovieScrobble scrobbleData = new TraktMovieScrobble
            {
                ...
                //Progress
                //Duration
                PluginVersion = MyFilmsSettings.Version.ToString(),
                MediaCenter = "mp-myfilms",
                MediaCenterVersion = MyFilmsSettings.MPVersion.ToString(),
                MediaCenterBuildDate = MyFilmsSettings.MPBuildDate.ToString("yyyy-MM-dd HH:mm:ss"),
                UserName = username,
                Password = password
            };
            return scrobbleData;
        ...
        public static void CheckTraktErrorAndNotify(TraktResponse response, bool notify)
        {
            ...
            if (response.Status != "success")
            {
                LogMyFilms.Debug("Trakt Error: {0}", response.Error);
                //if (notify) TVSeriesPlugin.ShowNotifyDialog(Translation.TraktError, response.Error);
            }
            else
            {
                // success
                LogMyFilms.Debug("Trakt Response: {0}", response.Message);
            }
        }
