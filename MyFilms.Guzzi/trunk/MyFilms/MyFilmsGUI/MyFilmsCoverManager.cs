...
namespace MyFilmsPlugin.MyFilms.MyFilmsGUI
{
    class MyFilmsCoverManager : GUIWindow
    {
        ...
        {            
            ...
            MediaPortal.GUI.Library.GUIPropertyManager.SetProperty("#currentmodule", Translation.Artwork);
            ...
            if (buttonFilters != null) buttonFilters.Label = Translation.ArtworkFilter;
            if (togglebuttonRandom != null)
            {
                togglebuttonRandom.Label = Translation.ButtonRandomArtwork;
        ...
        private void UpdateFilterProperty(bool btnEnabled)
        {
            ...
            TVSeriesPlugin.setGUIProperty("Artwork.FilterResolution", resolution);            
        }

        void worker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {            
            TVSeriesPlugin.setGUIProperty("Artwork.LoadingStatus", string.Empty);
            TVSeriesPlugin.setGUIProperty("Artwork.Count", totalFanart.ToString());

            if (totalFanart == 0)
            {
                TVSeriesPlugin.setGUIProperty("Artwork.LoadingStatus", Translation.ArtworkNoneFound);
        ...
        {            
            TVSeriesPlugin.setGUIProperty("Artwork.PageTitle", Title);
        }
        ...
        protected override void OnShowContextMenu()
        {
            try
            {
                ...
                if (dlg == null) return;
                dlg.Reset();
                dlg.SetHeading(Translation.Artwork);
            ...
            {
                MPTVSeriesLog.Write("Exception in Artwork Chooser Context Menu: " + ex.Message);
                return;
            }
        }
        #endregion
        ...
        private void ShowIntervalMenu() {
            ...
            dlg.Reset();
            dlg.SetHeading(Translation.ArtworkRandomInterval);

            GUIListItem pItem = new GUIListItem(Translation.ArtworkIntervalFiveSeconds);            
            dlg.Add(pItem);
            pItem.ItemId = (int)menuIntervalAction.FiveSeconds;

            pItem = new GUIListItem(Translation.ArtworkIntervalTenSeconds);
            dlg.Add(pItem);
            pItem.ItemId = (int)menuIntervalAction.TenSeconds;

            pItem = new GUIListItem(Translation.ArtworkIntervalFifteenSeconds);
            dlg.Add(pItem);
            pItem.ItemId = (int)menuIntervalAction.FifteenSeconds;

            pItem = new GUIListItem(Translation.ArtworkIntervalThirtySeconds);
            dlg.Add(pItem);
            pItem.ItemId = (int)menuIntervalAction.ThirtySeconds;

            pItem = new GUIListItem(Translation.ArtworkIntervalFortyFiveSeconds);
            dlg.Add(pItem);
            pItem.ItemId = (int)menuIntervalAction.FortyFiveSeconds;

            pItem = new GUIListItem(Translation.ArtworkIntervalSixtySeconds);
            dlg.Add(pItem);
        ...
        private void ShowFiltersMenu()
        {
            ...
            dlg.Reset();
            dlg.SetHeading(Translation.ArtworkFilter);

            GUIListItem pItem = new GUIListItem(Translation.ArtworkFilterAll);
            dlg.Add(pItem);
            ...
            try {
                if (m_Facade != null) {
                    ...
                    if (loadedItem != null) {                        
                        ...
                        TVSeriesPlugin.setGUIProperty("Artwork.LoadingStatus", string.Format(Translation.ArtworkOnlineLoading, e.ProgressPercentage, totalFanart));
                        TVSeriesPlugin.setGUIProperty("Artwork.Count", e.ProgressPercentage.ToString());
                        if (m_Facade != null) this.m_Facade.Focus = true;
                    }
                    else if (e.ProgressPercentage > 0) {
                        // we use this to tell the gui how many Artwork we are loading
                        TVSeriesPlugin.setGUIProperty("Artwork.LoadingStatus", string.Format(Translation.ArtworkOnlineLoading, 0, e.ProgressPercentage));
                        totalFanart = e.ProgressPercentage;
                    }
                }
            }
            catch (Exception ex) {
                MPTVSeriesLog.Write("Error: Artwork Chooser worker_ProgressChanged() experienced an error: " + ex.Message);               
            }
        }
        ...
        void loadThumbnails(int seriesID)
        {
            ...
            {                
                ...
                {                    
                    ...
                    {
                        item = new GUIListItem(Translation.ArtworkOnline);
                        item.IsRemote = false;
        ...
        void setFanartPreviewBackground(DBFanart fanart)
        {
            ...
            foreach (KeyValuePair<string, DBField> kv in fanart.m_fields)
            {
                switch (kv.Key)
                {
                    case DBFanart.cResolution:                 
                        TVSeriesPlugin.setGUIProperty("Artwork.SelectedArtworkResolution", kv.Value.Value);                        
                        break;

                    case DBFanart.cColors:
                        TVSeriesPlugin.setGUIProperty("Artwork.SelectedArtworkColors", kv.Value.Value);
                        break;

                    case DBFanart.cChosen:
                        TVSeriesPlugin.setGUIProperty("Artwork.SelectedArtworkIsChosen", kv.Value.Value?Translation.Yes:Translation.No);
                        break;

                    case DBFanart.cDisabled:
                        TVSeriesPlugin.setGUIProperty("Artwork.SelectedArtworkIsDisabled", kv.Value.Value ? Translation.Yes : Translation.No);
                        break;
            ...
            TVSeriesPlugin.setGUIProperty("Artwork.SelectedArtworkInfo", fanartInfo);
            ...
            TVSeriesPlugin.setGUIProperty("Artwork.SelectedPreview", preview);
        }
