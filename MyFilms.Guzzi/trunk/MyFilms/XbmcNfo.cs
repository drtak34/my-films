...
namespace MesFilms
{
    class XbmcNfo
    {
        ...
        public string ConvertXbmcNfo(string source, string folderimage, string foldermovies, bool SortTitle, bool OnlyFile, string TitleDelim)
        {
            if (TitleDelim.Length == 0)
                TitleDelim = "\\";
            string[] wStrStorage = MesFilms.conf.StrDirStor.ToString().Split(new Char[] { ';' });
