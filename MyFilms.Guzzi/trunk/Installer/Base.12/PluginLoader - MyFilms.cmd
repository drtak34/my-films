...
:SafetyCheck
	...
	IF EXIST "%ProgramFiles%\Team MediaPortal\MediaPortal\PluginConfigLoader.exe"  (
		start "MyFilms Config" "%ProgramFiles%\Team MediaPortal\MediaPortal\PluginConfigLoader.exe" /plugin="%ProgramFiles%\Team MediaPortal\MediaPortal\plugins\Windows\MyFilms.dll"
	) ELSE (
		IF EXIST "%ProgramFiles(x86)%\Team MediaPortal\MediaPortal\PluginConfigLoader.exe"  (
			start "MyFilms Config" "%ProgramFiles(x86)%\Team MediaPortal\MediaPortal\PluginConfigLoader.exe" /plugin="%ProgramFiles(x86)%\Team MediaPortal\MediaPortal\plugins\Windows\MyFilms.dll"
		) ELSE (
